...
uint64_t nLocalq3chainServices = 0;
...
/* MCHN START */
    ...
    nLocalq3chainServices |= pEF->NET_Services();
    if(mc_gState->m_NetworkParams->IsProtocolq3chain())
    {
        subver=FormatSubVersion("q3chain", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, subver, nBestHeight, true, nLocalq3chainServices);
/* MCHN END */
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (q3chain users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "q3chain " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. q3chain Core is probably already running."), addrBind.ToString());
        else
