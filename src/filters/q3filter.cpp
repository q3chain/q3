...
int mc_q3chainFilterParams::Zero()
{
    m_MaxShownData=-1;
...
int mc_q3chainFilterParams::Init()
{
    Zero();
...
int mc_q3chainFilterParams::Close()
{
    mc_gState->m_Compatibility=m_Compatibility;    
    return MC_ERR_NOERROR;
}

int mc_q3chainFilterParams::Destroy()
{
    return Zero();
}
...
int mc_q3chainLibrary::Initialize(const unsigned char* short_txid,uint32_t index)
{
    return MC_ERR_NOERROR;    
}

int mc_q3chainFilter::Zero()
{
...
int mc_q3chainFilter::Initialize(const unsigned char* short_txid)
{
    size_t value_size;
    unsigned char *ptr;
    ...
    if(ptr)
    {
        m_FilterCodeRow=pq3chainFilterEngine->m_CodeLibrary->GetNumElements();
        pq3chainFilterEngine->m_CodeLibrary->AddElement();
        pq3chainFilterEngine->m_CodeLibrary->SetData(ptr,value_size);
        pq3chainFilterEngine->m_CodeLibrary->SetData(&ntc,1);        
    }                                    
    else
    {
        m_FilterCodeRow=pq3chainFilterEngine->m_CodeLibrary->GetNumElements();
...
bool mc_q3chainFilter::HasRelevantEntity(set <uint160>& sRelevantEntities)
{
...
int mc_q3chainFilterEngine::Zero()
{
    m_Filters.clear();
    m_TxID=0;
    m_EntityTxID=0;
...
int mc_q3chainFilterEngine::Destroy()
{
    if(m_Semaphore)
    {
...
void mc_q3chainFilterEngine::Lock(int write_mode)
{        
...
void mc_q3chainFilterEngine::UnLock()
{    
    m_LockedBy=0;
    __US_SemPost(m_Semaphore);
}

int mc_q3chainFilterEngine::InFilter()
{    
...
int mc_q3chainFilterEngine::GetAcceptTimeout()
{    
    return GetArg("-acceptfiltertimeout",DEFAULT_ACCEPT_FILTER_TIMEOUT);
}

int mc_q3chainFilterEngine::GetSendTimeout()
{    
...
int mc_q3chainFilterEngine::SetTimeout(int timeout)
{
    Lock(1);
...
int mc_q3chainFilterEngine::SetTimeoutInternal(int timeout)
{
    for(int i=0;i<(int)m_Filters.size();i++)
    {
        mc_Filter *worker=*(mc_Filter **)m_Workers->GetRow(i);
...
uint160 mc_q3chainFilterEngine::ActiveUpdateID(uint160 hash)
{
    map<uint160,mc_q3chainLibrary>::iterator it=m_Libraries.find(hash);
...
int mc_q3chainFilterEngine::LoadLibrary(uint160 hash,bool *modified)
{
    int count;
    size_t value_size;
    unsigned char *ptr;
    ...
    for(int i=0;i<count;i++)
    {
        ...
        map<uint160,mc_q3chainLibrary>::iterator it=m_Libraries.find(library[i].m_Hash);
        if (it == m_Libraries.end())
        {
            if(i == 0)
            {
...
int mc_q3chainFilterEngine::CheckLibraries(set<uint160>* lpAffectedLibraries, int for_block)
{
    Lock(1);
...
int mc_q3chainFilterEngine::CheckLibrariesInternal(set<uint160>* lpAffectedLibraries, int for_block)
{
    int err;
    bool modified;
    map<uint160,mc_q3chainLibrary>::iterator mit;
    map<uint160,mc_q3chainLibrary>::iterator lit;    
...
mc_Filter *mc_q3chainFilterEngine::StreamFilterWorker(int row,bool *modified)
{
    int err; 
    char *code;
    size_t code_size;
    ...
    *modified=false;
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        ...
        map<uint160,mc_q3chainLibrary>::iterator it=m_Libraries.find(mc_LibraryIDForUpdate(m_Filters[row].m_Libraries[i]));
        if(it == m_Libraries.end())
        {
...
int mc_q3chainFilterEngine::RebuildFilter(int row,int for_block)
{
    int err; 
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        map<uint160,mc_q3chainLibrary>::iterator it=m_Libraries.find(m_Filters[row].m_Libraries[i]);
        if (it != m_Libraries.end())
        {
...
int mc_q3chainFilterEngine::AddFilter(const unsigned char* short_txid,int for_block)
{
    Lock(1);
...
int mc_q3chainFilterEngine::AddFilterInternal(const unsigned char* short_txid,int for_block)
{    
    int err;
    mc_q3chainFilter filter;
...
int mc_q3chainFilterEngine::Reset(int block,int for_block)
{
    Lock(1);
...
int mc_q3chainFilterEngine::ResetInternal(int block,int for_block)
{
    int filter_block;
    int err;
...
int mc_q3chainFilterEngine::NoStreamFilters()
{
    int ret=1;
...
int mc_q3chainFilterEngine::RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_q3chainFilter **lppFilter,int *applied)            
{
    if(mc_gState->m_Features->StreamFilters() == 0)
    {
        return MC_ERR_NOERROR;
...
int mc_q3chainFilterEngine::RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_q3chainFilter **lppFilter,int *applied,bool only_once)
{    
    Lock(0);
...
int mc_q3chainFilterEngine::RunFilter(const CTransaction& tx,mc_Filter *filter,std::string &strResult)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
int mc_q3chainFilterEngine::RunFilterWithCallbackLog(const CTransaction& tx,int vout,uint256 stream_txid,mc_Filter *filter,std::string &strResult, json_spirit::Array& callbacks)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
void mc_q3chainFilterEngine::SetCallbackNames()
{
    Lock(1);
...
void mc_q3chainFilterEngine::SetCallbackNamesInternal()
{
    m_CallbackNames.clear();
...
int mc_q3chainFilterEngine::Initialize()
{
    mc_Buffer *filters;
    unsigned char *txid;
    int err=MC_ERR_NOERROR;
