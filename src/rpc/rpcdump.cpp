...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    bool fNewFound=false;
    vector<string> inputStrings=ParseStringList(params[0]);
    vector<Cq3chainAddress> inputAddresses;
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        ...
        if (address.IsValid()) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid q3chain address or script");
        }
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        Cq3chainAddress address=inputAddresses[is];
        ...
        else
        {
            ...
            address=Cq3chainAddress(innerID);
        }
...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", Cq3chainAddress(keyid).ToString());
//                continue;
            fAlreadyHave=true;
        }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid q3chain address");
    CKeyID keyID;
    ...
    return Cq3chainSecret(vchSecret).ToString();
}
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by q3chain %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = Cq3chainAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", Cq3chainSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", Cq3chainSecret(key).ToString(), strTime, strAddr);
            } else {
