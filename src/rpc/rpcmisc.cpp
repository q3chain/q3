...
bool Cq3chainAddressFromTxEntity(Cq3chainAddress &address,mc_TxEntity *lpEntity);
Object AddressEntry(Cq3chainAddress& address,uint32_t verbose);
...
//    obj.push_back(Pair("protocolversion", PROTOCOL_VERSION));
    ...
    obj.push_back(Pair("nodeaddress", q3chainServerAddress(true) + strprintf(":%d",GetListenPort())));       
 ...
 */ 
    ...
    keyID=pkey.GetID();    
    obj.push_back(Pair("handshakelocal",GetArg("-handshakelocal",Cq3chainAddress(keyID).ToString())));   
...
Value setruntimeparam(const json_spirit::Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "This API is supported only if protocol=q3chain");                
    }
...
/* MCHN START */        
    if(!isValid)
    {
        ...
        if(isValid)
        {
            address=Cq3chainAddress(pubkey.GetID());
        }        
    }
...
Value createkeypairs(const Array& params, bool fHelp)
{
    ...
    for(int i=0;i<count;i++)
    {
        ...
        Object entry;
        entry.push_back(Pair("address", Cq3chainAddress(pubkey.GetID()).ToString()));
        entry.push_back(Pair("pubkey", HexStr(pubkey)));
        entry.push_back(Pair("privkey", Cq3chainSecret(secret).ToString()));
...
#ifdef ENABLE_WALLET
        // Case 1: q3chain address and we have full public key:
        Cq3chainAddress address(ks);
...
/* MCHN END */    
    ...
    Cq3chainAddress address(innerID);
...
Value getdiagnostics(const Array& params, bool fHelp)
{
    ...
    node.push_back(Pair("defaultaddress", Cq3chainAddress(pwalletMain->vchDefaultKey.GetID()).ToString()));
...
//    obj.push_back(Pair("", mc_gState->m_NetworkParams->GetInt64Param("")));    
    ...
    while((height>=0) && !found)
    {
        ...
        if(mc_gState->m_Permissions->GetBlockMiner(height,(unsigned char*)&keyID) == MC_ERR_NOERROR)
        {
            ...
            entry.push_back(Pair("miner",Cq3chainAddress(keyID).ToString()));
...

    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodes) 
        {
            ...
            if(stats.fSuccessfullyConnected)
            {
                ...
                {
                    obj.push_back(Pair("handshakelocal", Cq3chainAddress(stats.kAddrLocal).ToString()));                
                    obj.push_back(Pair("handshake", Cq3chainAddress(stats.kAddrRemote).ToString()));                
                }
