...
//        int period_pos=permission_type.find_last_of(".",permission_type.size());
    ...
    for(int i=0;i<permissions->GetCount();i++)
    {
        ...
        if(flags & MC_PFL_IS_SCRIPTHASH)
        {
            ...
            entry.push_back(Pair("address", Cq3chainAddress(lpScriptID).ToString()));                
        ...
        else
        {
            ...
            entry.push_back(Pair("address", Cq3chainAddress(lpKeyID).ToString()));
        }
            ...
            if(details)
            {
                for(int j=0;j<details->GetCount();j++)
                {
                    ...
                    if(remaining > 0)
                    {
                        ...
                        {
                            CScriptID lpScriptID=CScriptID(addr);
                            admins.push_back(Cq3chainAddress(lpScriptID).ToString());                                                                            
                        }
                        else
                        {
                            CKeyID lpKeyID=CKeyID(addr);
                            ...
                            if(remaining == 0)
                            {
                                if(block_from == plsPend->m_BlockFrom)
                                {
                                    if(block_to == plsPend->m_BlockTo)
                                    {
                                        ...
                                        {
                                            CScriptID lpScriptID=CScriptID(addr);
                                            pend_admins.push_back(Cq3chainAddress(lpScriptID).ToString());                                                                            
                                        }
                                        else
                                        {
                                            CKeyID lpKeyID=CKeyID(addr);
...
Array mc_GetRecentGrants(mc_PermissionDetails *plsRow,uint32_t from_block)
{
    ...
    if(details)
    {
        for(int j=details->GetCount()-1;j>=0;j--)
        {
            ...
            {
                CScriptID lpScriptID=CScriptID(addr);
                entry.push_back(Pair("admin",Cq3chainAddress(lpScriptID).ToString()));                
            }
            else
            {
                CKeyID lpKeyID=CKeyID(addr);
                entry.push_back(Pair("admin",Cq3chainAddress(lpKeyID).ToString()));
            }
...
// 0x02 - check last changes
    ...
    for(int i=0;i<(int)admins.size();i++)
    {
        ...
        {
            entry.push_back(Pair("address",Cq3chainAddress((CScriptID)(admins[i].m_Address)).ToString()));            
            entry.push_back(Pair("islocal",false));
        }
        else
        {
            bool is_mine=(IsMine(*pwalletMain, (CKeyID)(admins[i].m_Address))  == ISMINE_SPENDABLE);
            entry.push_back(Pair("address",Cq3chainAddress((CKeyID)(admins[i].m_Address)).ToString()));
            entry.push_back(Pair("islocal",is_mine));
        }
...
// 0x02 - check last changes
    ...
    for(int i=0;i<(int)miners.size();i++)
    {
        ...
        {
            entry.push_back(Pair("address",Cq3chainAddress((CScriptID)(miners[i].m_Address)).ToString()));            
            entry.push_back(Pair("islocal",is_mine));
        }
        else
        {
            is_mine=(IsMine(*pwalletMain, (CKeyID)(miners[i].m_Address))  == ISMINE_SPENDABLE);
            entry.push_back(Pair("address",Cq3chainAddress((CKeyID)(miners[i].m_Address)).ToString()));
            entry.push_back(Pair("islocal",is_mine));
        }
