...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "q3chain server stopping";
}
...
Array JSONRPCExecInternalBatch(const Array& vReq)
{
    ...
    for (unsigned int reqIdx = 0; reqIdx < vReq.size(); reqIdx++)
    {
        ...
        {
            take_it=false;
            if(defaultaddress == Cq3chainAddress(pwalletMain->vchDefaultKey.GetID()).ToString())
            {
                take_it=true;
            }
        }        
...
bool  HTTPReq_JSONRPC(string& strRequest, uint32_t flags, string& strReply, string& strHeader, map<string, string>& mapHeaders,int& http_code,json_spirit::Value& valError,json_spirit::Value& req_id)
{
    ...
    mapHeaders.insert(make_pair("Server",strprintf("q3chain-json-rpc/%s",FormatFullq3chainVersion())));
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params, const Value& req_id) const
{
    ...
    if (!pcmd)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Method not available in cold version of q3chain");            
        }
    }
#ifdef ENABLE_WALLET
    if (pcmd->reqWallet && !pwalletMain)                                        // Never happens, reqWallet is changed to require Wallet read lock 
    ...
    if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
    {
        ...
        {
            throw JSONRPCError(RPC_UPGRADE_REQUIRED, strprintf("BlockChain was upgraded to protocol version %d, please upgrade q3chain",mc_gState->m_ProtocolVersionToUpgrade));
        }
    }
...
std::string HelpExampleCli(string methodname, string args){
    return "> q3chain-cli " + std::string(mc_gState->m_NetworkParams->Name()) + " " + methodname + " " + args + "\n";
}
