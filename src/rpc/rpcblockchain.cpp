...
/* MCHN START */
bool Parseq3chainTxOutToBuffer(uint256 hash,const CTxOut& txout,mc_Buffer *amounts,mc_Script *lpScript,int *allowed,int *required,string& strFailReason);
...
/* MCHN START */    
    ...
    if(mc_gState->m_NetworkParams->IsProtocolq3chain())
    {
        if (chainActive.Contains(blockindex))
        {
            ...
            {
                miner=Cq3chainAddress(keyID).ToString();
            }
        }
        else
        {
...
//        throw JSONRPCError(RPC_INVALID_PARAMS, "Wrong number of parameters");                    
    ...
    if( (vin < 0) || (vin >= (unsigned int)pq3chainFilterEngine->m_Tx.vin.size()) )
    {
    ...
    Array getxoutparams;
    getxoutparams.push_back(pq3chainFilterEngine->m_Tx.vin[vin].prevout.hash.ToString());
    getxoutparams.push_back((int64_t)pq3chainFilterEngine->m_Tx.vin[vin].prevout.n);
...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    {
        if (!((CCoinsViewCache*)(pq3chainFilterEngine->m_CoinsCache))->GetCoins(hash, coins))
            return Value::null;        
    }
    else
    {
        if (fMempool) {
            ...
            if (!view.GetCoins(hash, coins))
                return Value::null;
            if(pq3chainFilterEngine->InFilter() == 0)                                // In filter we already checked this input exists, but mempool is dirty
            {
