...
Value getmultibalances_operation(const Array& params, bool fHelp,bool aggregate_tokens)
{
    ...
    if(params.size() > 0)
    {
        ...
        {        
            ...
            {
                Cq3chainAddress address(str_addr);
                dest=address.Get();
    ...
    
    {
        ...
        {        
            ...
            str_addr=Cq3chainAddress(address).ToString();
    ...
    if (params.size() > 0)
    {
        if (params[0].get_str() != "*") 
        {
            if (params[0].get_str() != "") 
            {
                ...
                {
                    throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getassetbalances, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
                }            
            }
            BOOST_FOREACH(const PAIRTYPE(Cq3chainAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
            {
...
//                        aMetaData.push_back(OpReturnEntry(elem,elem_size,wtx.GetHash(),i));
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& item, mAddresses)
    {
        ...
        {
            oBalance.push_back(Pair(Cq3chainAddress(item.first).ToString(), units*item.second));            
        }
    }
    if(other_amount != 0)
    {
...
Value getfilterassetbalances_operation(const Array& params, bool fHelp,bool aggregate_tokens)
{
    ...
    for(int j=0;j<(int)pq3chainFilterEngine->m_Tx.vin.size();j++)
    {
        int n=pq3chainFilterEngine->m_Tx.vin[j].prevout.n;
        CCoins coins;
        {
            if(pq3chainFilterEngine->m_CoinsCache)
            {
                if (!((CCoinsViewCache*)(pq3chainFilterEngine->m_CoinsCache))->GetCoins(pq3chainFilterEngine->m_Tx.vin[j].prevout.hash, coins))
                    return Value::null;        
            }
            else
            {
                ...
                if (!view.GetCoins(pq3chainFilterEngine->m_Tx.vin[j].prevout.hash, coins))
                {
                    return Value::null;                                                                 
                }
            }
                    ...
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        if(aggregate_tokens)
                        {
                            ...
                            else
                            {
                                ...
                                if(memcmp(lpAsset->GetTxID(),&(pq3chainFilterEngine->m_Tx.vin[gin].prevout.hash),sizeof(uint256)) == 0)
                                {
    ...
    for (int j = 0; j < (int)pq3chainFilterEngine->m_Tx.vout.size(); ++j)
    {
        const CTxOut& txout = pq3chainFilterEngine->m_Tx.vout[j];
        if(!txout.scriptPubKey.IsUnspendable())
        {
            CTxDestination address;
            int required=0;
                ...
                if(CreateAssetBalanceList(txout,asset_amounts,lpScript,&required,aggregate_tokens))
                {
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        if(aggregate_tokens)
                        {
                            ...
                            {
                                if(memcmp(lpAsset->GetTxID(),&(pq3chainFilterEngine->m_TxID),sizeof(uint256)) == 0)
                                {
                                    if( mc_GetABRefType(asset_amounts->GetRow(i)) == MC_AST_ASSET_REF_TYPE_GENESIS )                    
                                    {
                                        quantity=mc_GetABQuantity(asset_amounts->GetRow(i));
    ...
    if(aggregate_tokens)
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& item, mAddresses)
        {
            ...
            {
                oBalance.push_back(Pair(Cq3chainAddress(item.first).ToString(), raw_value ? item.second : item.second/multiple));            
            }
        }
        if(other_amount != 0)
        {
    ...
    else
    {
        ...
        for(auto const &token_item : mTokenAddresses)
        {
            ...
            BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& item, token_item.second)
            {
                ...
                {
                    oBalance.push_back(Pair(Cq3chainAddress(item.first).ToString(), item.second));            
                }
            }
            if(other_amount != 0)
            {
...
Value gettokeninfo(const Array& params, bool fHelp)
{
    ...
    {
         throw JSONRPCError(RPC_NOT_SUPPORTED, "The node was created with previous version of q3chain. To use this API, please restart q3chaind with -reindex.");                       
    }
