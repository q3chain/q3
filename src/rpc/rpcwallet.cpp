...
bool mc_GetDefaultLicenseAddress(Cq3chainAddress& license_address);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need accounts, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
        }            
    ...
    return Cq3chainAddress(keyID).ToString();
}


Cq3chainAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return Cq3chainAddress(account.vchPubKey.GetID());
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    Cq3chainAddress address(params[0].get_str());
...
Value listaddresses(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "API is not supported with this wallet version. To get this functionality, run \"q3chaind -walletdbversion=2 -rescan\" ");        
    }   
    ...
    Cq3chainAddress license_address;
    ...
    for(int i=0;i<entity_count;i++)
    {
            ...
            ((lpEntity->m_Flags & MC_EFL_LICENSE) == 0 ) )               
        {
            if(setAddresses.size())
            {
                ...
                if(Cq3chainAddressFromTxEntity(address,&(lpEntity->m_Entity)))
                {
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(Cq3chainAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(Cq3chainAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(Cq3chainAddress(address).Get())->second.name);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CKey key;
    Cq3chainAddress addr(strAddress);
    if (!addr.IsValid())
    {
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need accounts, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }            
    
    // q3chain address
    Cq3chainAddress address = Cq3chainAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getbalance, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    string strAccount = AccountFromValue(params[0]);
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need move, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendfrom, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    
    string strAccount = AccountFromValue(params[0]);
    Cq3chainAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    CAmount nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendmany, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    ...
    set<Cq3chainAddress> setAddress;
    vector<pair<CScript, CAmount> > vecSend;

    CAmount totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        Cq3chainAddress address(s.name_);
        if (!address.IsValid())
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return Cq3chainAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<Cq3chainAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(Cq3chainAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<Cq3chainAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listtransactions(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need listtransactions, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    else
    {
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need listaccounts, run q3chaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }   
