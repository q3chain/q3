 ...
 */ 
    ...
    {
//        if(pq3chainFilterEngine->m_TxID != 0)
        if(pq3chainFilterEngine->InFilter())
        {
...
/* MCHN END */
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(Cq3chainAddress(addr).ToString());
...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be Accepq3chainTransaction
...
int VerifyNewTxForStreamFilters(const CTransaction& tx,std::string &strResult,mc_q3chainFilter **lppFilter,int *applied)            
{
    ...
    if(pq3chainFilterEngine->NoStreamFilters())
    {
        return MC_ERR_NOERROR;
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) 
    {
        ...
        if(result.type() == obj_type)
        {
            ...
            int err=pq3chainFilterEngine->RunStreamFilters(tx,i,(unsigned char*)&hash+MC_AST_SHORT_TXID_OFFSET,-1,0, 
...
//        throw runtime_error("Help message not found\n");
    ...
    if(pq3chainFilterEngine->m_Vout < 0)
    {
    ...
    if(pq3chainFilterEngine->m_Vout >= (int)pq3chainFilterEngine->m_Tx.vout.size())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vout out of range");                                            
    }
    Value result=DataItemEntry(pq3chainFilterEngine->m_Tx,pq3chainFilterEngine->m_Vout,streams_already_seen, 0x0E00);
...
//    result.get_obj().push_back(Pair("txid", pq3chainFilterEngine->m_Tx.GetHash().GetHex()));
    result.get_obj().push_back(Pair("vout", pq3chainFilterEngine->m_Vout));
...
Value getfilterstream(const Array& params, bool fHelp)
{
    ...
    Value result=StreamEntry((unsigned char*)&(pq3chainFilterEngine->m_EntityTxID),0x0002);
...
*/    
    Object result;
    TxToJSON(pq3chainFilterEngine->m_Tx, 0, result);
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH(Value& input, inputs) {
            Cq3chainAddress address(input.get_str());
            if (!address.IsValid())
...
/* MCHN END */       
        ...
        CTxDestination address;
        if (ExtractDestination(txout.scriptPubKey, address)) {
            entry.push_back(Pair("address", Cq3chainAddress(address).ToString()));
...
/* MCHN START */                
                                                                                // q3chain issue #6056 with booust 1.58 fixed in https://github.com/q3chain/q3chain/commit/8b08d9530b93c7a98e7387167ecd2cd5b0563bfb
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(Parseq3chainTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(Parseq3chainTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=q3chain");
    }    
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", Cq3chainAddress(CScriptID(script)).ToString()));
    return r;
}
