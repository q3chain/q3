...
using namespace std;
void Pushq3chainRelay(CNode* pto, uint32_t msg_type,vector<CAddress>& path,vector<CAddress>& path_to_follow,vector<unsigned char>& payload);
...
Value liststorednodes(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(PAIRTYPE(CService, vector<CMCAddrInfo>) naddr, mNetAddresses)
    {
        ...
        BOOST_FOREACH(CMCAddrInfo info, naddr.second)
        {
            ...
            {
                handshake.push_back(Pair("address",Cq3chainAddress((CKeyID)info.GetMCAddress()).ToString()));
...
//                    Pushq3chainRelay(pnode, MC_RMT_GLOBAL_PING,path,path_to_follow,payload);
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolq3chain())
    {
        obj.push_back(Pair("subversion",
            FormatSubVersion("q3chain", mc_gState->GetProtocolVersion(), std::vector<string>())));
    }
    else
    {
        obj.push_back(Pair("subversion",
