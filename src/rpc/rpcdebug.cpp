...
Object mcd_AddrMan()
{
    ...
    BOOST_FOREACH(PAIRTYPE(CService, vector<CMCAddrInfo>) naddr, m_NetAddresses)
    {
        ...
        BOOST_FOREACH(CMCAddrInfo info, naddr.second)
        {
            ...
            if(info.GetMCAddress() != 0)
            {
                entry.push_back(Pair("mcaddress",Cq3chainAddress((CKeyID)info.GetMCAddress()).ToString()));
            }
            else
            {
    ...
    BOOST_FOREACH(PAIRTYPE(uint160, vector<CMCAddrInfo>) maddr, m_MCAddresses)
    {
        ...
        BOOST_FOREACH(CMCAddrInfo info, maddr.second)
        {
            ...
            string smaddr=Cq3chainAddress((CKeyID)maddr.first).ToString();
...
Value mcd_DebugIssueLicenseToken(const Object& params)
{
    ...
    Cq3chainAddress from_address(mcd_ParamStringValue(params,"from_address",""));
    if (!from_address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    Cq3chainAddress to_address(mcd_ParamStringValue(params,"to_address",""));
    if (!to_address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    Cq3chainAddress req_address(mcd_ParamStringValue(params,"req_address",""));
...
//        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid request type");                    
    ...
    else
    {
        switch(request_type)
        {
            case MC_RMT_MC_ADDRESS_QUERY:
                ...
                Cq3chainAddress address(addr_to_find);
                CKeyID keyID;
    ...
    if(request_type & MC_RMT_SPECIAL_MASK)
    {
        ...
        for(int a=0;a<attempts;a++)
        {
            ...
            remaining=q3chainCollectChunks(&collector);
            if(remaining == 0)
            {
                return 0;
            }
            ...
            if(params.size() > 3)
            {
                ...
                BOOST_FOREACH(CNode* pnode, vNodes) 
                {
                    ...
                    if( (params[3].get_str() == stats.addrName) || 
                        (params[3].get_str() == Cq3chainAddress(stats.kAddrRemote).ToString()) )
                    {
                        pto=pnode;
                    }
                }
                    ...
                    case MC_RMT_MC_ADDRESS_QUERY:
                        ...
                        for(int r=0;r<(int)request->m_Responses.size();r++)
                        {
                            if(!res_found)
                            {
                                ...
                                {
                                    Array addresses;
                                    res.push_back(Pair("handshake",Cq3chainAddress(node_addr.m_Address).ToString()));
