...
bool Processq3chainRelay(CNode* pfrom, CDataStream& vRecv, CValidationState &state);
bool Processq3chainVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool Pushq3chainVerack(CNode* pfrom, bool fIsVerackack);
bool q3chainNode_CanConnect(CNode *pnode);
bool q3chainNode_DisconnectRemote(CNode *pnode);
bool q3chainNode_DisconnectLocal(CNode *pnode);
bool q3chainNode_RespondToGetData(CNode *pnode);
bool q3chainNode_SendInv(CNode *pnode);
bool q3chainNode_AcceptData(CNode *pnode);
bool q3chainNode_IgnoreIncoming(CNode *pnode);
bool q3chainNode_IsLocal(CNode *pnode);
bool q3chainNode_CollectChunks();
...
#if defined(NDEBUG)
# error "q3chain cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "q3chain Signed Message:\n";
...
int SetUpgradedParamValue(const mc_Oneq3chainParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        Setq3chainParam("targetblocktime",value);
    }   
...
int q3chainNode_ApplyUpgrades(int current_height)
{
    ...
    Setq3chainParams();            
    if(pq3chainFilterEngine)                                                 // Added from version 20012. 
    {
        pq3chainFilterEngine->SetCallbackNames();
    }
...
void q3chainNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool q3chainNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(!Acceptq3chainTransaction(tx,view,-1,MC_AMT_DEFAULT,reason, &mandatory_fee, &replay))
        {
            return state.DoS(0,
                             error("AcceptToMemoryPool: : Acceptq3chainTransaction failed %s : %s", hash.ToString(),reason),
                             REJECT_NONSTANDARD, reason);
        }
...
*/
        ...
        CAmount nFees = 0;
        int async_count=0;
        bool fIsLicenseTokenTransfer=Is_q3chainLicenseTokenTransfer(tx);
...
/* MCHN START */        
    if(!fJustCheck)
    {
        ...
        {
            pq3chainFilterEngine->Reset(pindex->nHeight-1,1);
        }
    }
/* MCHN END */        
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            const CTransaction &tx = block.vtx[i];
            string reason;
            if(!Acceptq3chainTransaction(tx,view,offset,MC_AMT_DEFAULT,reason,NULL,NULL))
            {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                ...
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : Acceptq3chainTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
                }
            }
/* MCHN END */                    
...
/* MCHN START */        
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(!Acceptq3chainTransaction(tx,view,coinbase_offset,MC_AMT_DEFAULT,reason,NULL,NULL))
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : Acceptq3chainTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    {
        Cq3chainAddress addr=Cq3chainAddress(chainActive.Tip()->kMiner.GetID());
...
/* MCHN START */    
    ...
    if(pq3chainFilterEngine)
    {
        pq3chainFilterEngine->Reset(old_height-1,0);
    }
    
    q3chainNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/*    
    if(pq3chainFilterEngine)
    {
        pq3chainFilterEngine->Reset(pindexNew->nHeight-1,1);
    }
 */ 
    ...
    }
    {
        ...
        if(pq3chainFilterEngine)
        {
            pq3chainFilterEngine->m_CoinsCache=&view;
        }
        bool rv = ConnectBlock(*pblock, state, pindexNew, view);        
        if(pq3chainFilterEngine)
        {
            pq3chainFilterEngine->m_CoinsCache=NULL;
        }
        if(rv)
        {
...
/* MCHN START */        
    ...
    q3chainNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
//    if (!fInvalidFound)
    {
        ...
        if(pq3chainFilterEngine)
        {
            pq3chainFilterEngine->Reset(chainActive.Height(),0);
        }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    q3chainNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            q3chainNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */
    ...
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
        ...
        if(pwalletTxsMain->RollBack(NULL,chainActive.Height()) != MC_ERR_NOERROR)
        {
            ...
            {
                strError="Error: The wallet database is inconsistent. Restart q3chain with -rescan to rebuild the wallet database from the blockchain (can take hours), or with -skipwalletchecks to continue operating anyway";
                return false;
            }
        }
    }
...
/* MCHN END */        
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if(pq3chainFilterEngine)
            {
                pq3chainFilterEngine->m_CoinsCache=&coins;
            }            
...
/* MCHN START */    
    if(!q3chainNode_RespondToGetData(pfrom)) 
    {
...
//    RandAddSeedPerfmon();
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->nq3chainServices=0;
        if (!vRecv.empty())
        {
            vRecv >> pfrom->nq3chainServices;
        }
...
/* MCHN START */
        ...
        if(mc_gState->m_NetworkParams->IsProtocolq3chain())
        {            
            if(GetBoolArg("-q3chainstylehandshake", false))
            {
                LogPrintf("mchn: q3chain-style-handshake, sending empty verack to peer %d... \n", pfrom->id);
                pfrom->fParameterSetVerified=true;
                pfrom->PushMessage("verack");                
            }
            else
            {
                Pushq3chainVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolq3chain())
        {
            if(vRecv.size() == 0)
            {
                ...
                else
                {
                    ...
                    {
                        LogPrintf("mchn: q3chain-style verack received from peer %d, parameter set NOT VERIFIED, connecting... \n", pfrom->id);
                        pfrom->fParameterSetVerified=true;                    
                        CompleteProcessVersion(pfrom);                
                    }
                }
            ...
            else
            {
                if(GetBoolArg("-q3chainstylehandshake", false))
                {
                    LogPrintf("mchn: q3chain-style-handshake, ignoring verack from peer %d \n", pfrom->id);                    
                }
                else
                {
                    bool disconnect_flag=false;
                    if(!Processq3chainVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !Pushq3chainVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolq3chain())
        {
            ...
            if(!Processq3chainVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !Pushq3chainVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!q3chainNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !q3chainNode_IgnoreIncoming(pfrom))      // MCHN
...
//        LOCK(cs_main);
    ...
    else if (strCommand == "tx" && q3chainNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!q3chainNode_IgnoreIncoming(pfrom) && !pfrom->fDisconnect)
        {
/* MCHN END */        
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && q3chainNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && q3chainNode_SendInv(pfrom))          // MCHN
    {
...
//        LOCK(cs_main);
        ...
        if(!pfrom->fDisconnect)
        {
            if(q3chainNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
                    LogPrintf("mchn: Address %s lost connect permission on peer=%d, diconnecting...\n",Cq3chainAddress(pfrom->kAddrRemote).ToString().c_str(), pfrom->id);            

            }
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && q3chainNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
                        LogPrintf("mchn: Local address %s lost connect permission. disconnecting peer %d...\n",Cq3chainAddress(pfrom->kAddrLocal).ToString().c_str(), pfrom->id);            
                }
            }
        }
    }
...
/* MCHN END */            
        ...
        try
        {
            if(pfrom->fDisconnect || !q3chainNode_DisconnectRemote(pfrom))
            {
 ...
 */ 
        ...
        if (!vInv.empty())
            if(q3chainNode_SendInv(pto))                                     // MCNN
                pto->PushMessage("inv", vInv);
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=q3chainNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && q3chainNode_IsBlockChainSynced(pto))
            {
...
/* MCHN START */        
        ...
        if(pwalletTxsMain->m_ChunkCollector)
        {
            ...
            {
                if(q3chainNode_CollectChunks())
                {
                    q3chainCollectChunks(pwalletTxsMain->m_ChunkCollector);
                }                
                pwalletTxsMain->m_ChunkCollector->m_NextTryTimestamp=time_millis_now+q3chainCollectChunksQueueStats(pwalletTxsMain->m_ChunkCollector);
