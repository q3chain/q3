...
mc_q3chainFilterEngine* pq3chainFilterEngine = NULL;
...
/* MCHN START */  
    ...
    {
        delete pq3chainFilterEngine;
        pq3chainFilterEngine=NULL;        
    }
...
std::string HelpMessage(HelpMessageMode mode)                                   // MCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "q3chain.conf") + "\n";
    if (mode == HMM_q3chainD)
    {
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "q3chain.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default q3chain address otherwise") + "\n";
#endif
/* MCHN START */    
/* Default was 0 */    
...
//    strUsage += "  -blockprioritysize=<n> " + strprintf(_("Set maximum size of high-priority/low-fee transactions in bytes (default: %d)"), DEFAULT_BLOCK_PRIORITY_SIZE) + "\n";
    ...
    strUsage += "\n" + _("q3chain runtime parameters") + "\n";    
    strUsage += "  -offline                                 " + _("Start q3chaind in offline mode, no connections to other nodes.") + "\n";
    strUsage += "  -storeruntimeparams                      " + _("Permanently save modifications to runtime parameters made by setruntimeparam APIs.") + "\n";
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running q3chaind for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a q3chain blockchain.") + "\n";
    ...
    strUsage += "  -explorersupport=0|2                     " + _("Provide support for q3chain Explorer 2, default 0") + "\n";

    strUsage += "\n" + _("q3chain API response parameters") + "\n";        
    strUsage += "  -hideknownopdrops      " + strprintf(_("Remove recognized q3chain OP_DROP metadata from the responses to JSON-RPC calls (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -v1apicompatible       " + strprintf(_("JSON-RPC calls responses compatible with q3chain 1.0 (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) Coin Sciences Ltd - www.q3chain.com")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.q3chain.com/terms-of-service/")) +
           "\n";
...
/** Sanity checks
 *  Ensure that q3chain is running in a usable environment with all
        ...
        if(pwalletMain)
        {
            if(pwalletMain->vchDefaultKey.IsValid())
            {
                LogPrintf("mchn: Minimal blockchain parameter set is created, default address: %s\n",
                        Cq3chainAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                ...
                {    
                    ...
                    sprintf(bufOutput,"q3chain-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                         Cq3chainAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                    sprintf(bufOutput,"q3chain-cli %s grant %s connect,send,receive\n\n",mc_gState->m_NetworkParams->Name(),
                         Cq3chainAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                }
                else
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. q3chain Core is shutting down."));
...
#endif
    // Make sure only a single q3chain process is using the data directory.
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. q3chain Core is probably already running."), strDataDir));
#endif
/* MCHN END */
...
//#endif
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
//    LogPrintf("q3chain version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolq3chain())
    {
        LogPrintf("q3chain version build %s protocol %s (%s)\n", mc_BuildDescription(mc_gState->GetNumericVersion()), mc_gState->GetProtocolVersion(), CLIENT_DATE);
    }
    ...
    if (!fDisableWallet) {
        ...
        if (filesystem::exists(pathWalletDat))
        {
            ...
            if(currentwalletdatversion != 1)
            {
                ...
                {
                    return InitError(strprintf("Wallet version %d is not supported in this edition of q3chain.\n\n"
                            "To upgrade to version %d, run q3chain Offline Daemon: \n"
                            "q3chaind-cold %s -datadir=%s -walletdbversion=3\n"
                            "and restart q3chaind or use Community Edition.\n",
                ...
                {
                    return InitError(strprintf("Wallet version %d is not supported in this build of q3chain.\n\n"
                            "To upgrade to version %d, run q3chain Daemon from official build: \n"
                            "q3chaind %s -walletdbversion=%d\n",
            ...
            if( currentwalletdatversion == 2 )
            {
                ...
                {
                    return InitError(strprintf("Wallet version %d is not supported in this version of q3chain.\n\n"
                            "To upgrade to version %d, please run \n"
                            "q3chaind %s -walletdbversion=3\n",
...
//            if(pEF->ENT_MinWalletDatVersion() > currentwalletdatversion)
        ...
        switch(currentwalletdatversion)
        {
            ...
            case 2:
                break;
            case 1:
                return InitError(strprintf("Wallet version 1 is not supported in this version of q3chain. "
                        "To upgrade to version 2, run q3chain 1.0: \n"
                        "q3chaind %s -walletdbversion=2 -rescan\n",mc_gState->m_NetworkParams->Name()));                                        
            default:
...
//            if (!CWalletDB::Recover(bitdbwrap, strWalletFile, true))
            if(!WalletDBRecover(bitdbwrap,strWalletFile,true))
                return false;
            sprintf(bufOutput,"\nTo work properly with salvaged addresses, you have to call importaddress API and restart q3chain with -rescan\n\n");
...
//        uiInterface.InitMessage.connect(SetRPCWarmupStatus);
    ...
    uiInterface.InitMessage(_("Initializing q3chain..."));
    ...
    {    
        ...
        if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
        {
            ...
            return InitError(_("wallet.dat is partially corrupted. Please try running q3chain with -salvagewallet."));                            
        }
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to q3chain 1.0 beta 2 or later"));                                                        
                        }
...
*/
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_resolved.c_str()))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart q3chaind";
...
//                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d - please check q3chaind is running at that address and that your firewall settings allow incoming connections.",                
                            seed_ip.c_str(),seed_port);
                        pNodeStatus->sLastError="Couldn't connect to the seed node, q3chaind is not running or firewall issue";
                    }
                }
            }
            else
            ...
            if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
            {
                ...
                if(mc_gState->IsSupported(protocol_version) == 0) 
                {
                    ...
                    {
                        seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in q3chain %s\n",                
                    ...
                    else
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d.\n"
                                    "Blockchain was created by q3chaind with newer protocol version (%d)\n"                
                                    "Please upgrade to the latest version of q3chain or connect only to blockchains using protocol version %d or earlier.\n",                
            ...
            if(mc_gState->m_NetworkState == MC_NTS_SEED_NO_PARAMS)
            {
                ...
                seed_error=strprintf("Couldn't retrieve blockchain parameters from the seed node %s on port %d.\n"
                            "For q3chain protocol blockchains, the file %s must be copied manually from an existing node.",                
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
            {
                Selectq3chainParams(mc_gState->m_Params->NetworkName());
        ...
        else
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_GENERATED)
            {
                ...
                mc_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocolq3chain flag in case of q3chain
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart q3chaind with reindex=1.\n",mc_gState->m_Params->NetworkName());
        ...
        else
        {
            ...
            if(mc_gState->m_WalletMode != MC_WMD_NONE)
            {
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart q3chaind with -rescan\n");                        
                }
                if((pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_EXPLORER_MASK) == MC_WMD_EXPLORER1)
                {
                    return InitError("-explorersupport=1 is not supported by this version of q3chain. To change it, please restart q3chaind with -rescan -explorersupport=2\n");                                                                                
                }
                if(explorer_mode >= 0)
                {                    
                    if( (pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_EXPLORER_MASK) != (mc_gState->m_WalletMode & MC_WMD_EXPLORER_MASK) )
                    {
                        return InitError("Explorer database was initialized with different setting. To change it, please restart q3chaind with -rescan\n");                                                    
                    }
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart q3chaind with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        
                ...
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart q3chaind with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after q3chain parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
            ...
            if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
            {
                ...
                return InitError(_("wallet.dat corrupted. Please try running q3chain with -salvagewallet."));                            
            }
        ...
        if(pwalletMain)
        {
            ...
            if(new_wallet_txs)
            {
                ...
                BOOST_FOREACH(const PAIRTYPE(Cq3chainAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
                {
                    const Cq3chainAddress& address = item.first;
...
/* MCHN START */    
    ...
    pq3chainFilterEngine=new mc_q3chainFilterEngine;
    if(pq3chainFilterEngine->Initialize())
    {
    ...
    if(!GetBoolArg("-shortoutput", false))
    {
        if(fListen && !GetBoolArg("-offline",false))
        {
            ...
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            sprintf(bufOutput,"q3chaind %s:%d\n\n",q3chainServerAddress(false).c_str(),GetListenPort());
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            if(found_ips > 1)
            {
        ...
        else
        {
            if(GetBoolArg("-offline",false))
            {                
                sprintf(bufOutput,"q3chain started in offline mode, other nodes cannot connect.\n\n");
                bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));            
            }
            else
            {
    ...
    {
        sprintf(bufOutput,"%s:%d\n",q3chainServerAddress(true).c_str(),GetListenPort());                
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
    }
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart q3chaind with reindex=1."),
...
//    int version=mc_gState->m_NetworkParams->GetInt64Param("protocolversion");
    ...
    LogPrintf("q3chain protocol version: %d\n",version);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of q3chain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart q3chain Core to complete") << "\n";
...
//    RandAddSeedPerfmon();
    ...
    // Generate coins in the background
        if (pwalletMain)
            Generateq3chains(GetBoolArg("-gen", true), pwalletMain, GetArg("-genproclimit", 1));
#endif
