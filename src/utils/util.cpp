...
#include "q3chain/q3chain.h"                                              // MCHN
...
/* MCHN START */                            
// Ignoring arguments not starting with "-". original q3chain code discard everything after that                
...
/* MCHN START */
static boost::filesystem::path pathCachedq3chain;
static boost::filesystem::path pathCachedq3chainLog;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Mac: ~/Library/Application Support/q3chain
    // Unix: ~/.q3chain
/* MCHN START */
    LOCK(csPathCached);
    
    pathCachedq3chain=fs::path(string(mc_gState->m_Params->DataDir()));
    fs::path &path =pathCachedq3chain;
    return path;
...
/* MCHN START */
    fs::path &path =pathCachedq3chainLog;
    if (!path.empty())
        return path;
    path=fs::path(string(mc_gState->m_Params->DataDir(2,1)));
    return pathCachedq3chainLog;
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No q3chain.conf file is OK
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "q3chain.pid"));
