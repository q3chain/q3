...
bool Parseq3chainTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
...
bool Parseq3chainTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
                                  ...
                                  string& strFailReason)                        // OUT error
{
    return Parseq3chainTxOutToBuffer(hash,txout,amounts,lpScript,allowed,required,NULL,strFailReason);
}
...
bool AddressCanReceive(CTxDestination address)
{
    ...
    Cq3chainAddress addressPrint;
    if(lpKeyID != NULL)
    {
        addressPrint=Cq3chainAddress(*lpKeyID);
        ptr=(unsigned char*)(lpKeyID);
    }
    else
    {
        addressPrint=Cq3chainAddress(*lpScriptID);
