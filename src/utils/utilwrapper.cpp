...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\q3chain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\q3chain
    // Mac and Unix: ~/.q3chain
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "q3chainCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "q3chain";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".q3chain-cold";        
    }
    return pathRet / ".q3chain";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"q3chain",".conf") == 0)
    {
                    ...
                    const char *prefix)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override q3chain.conf
                    ...
                    const char *prefix)
{
    try
    {
        ...
        if (!streamConfig.good())
            return MC_ERR_NOERROR; // No q3chain.conf file is OK
...
int mc_Params::ReadConfig(const char *network_name)
{
    ...
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"q3chain",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_q3chainParams::SetProtocolGlobals()
{
...
int mc_q3chainParams::SetGlobals()
{
    m_IsProtocolq3chain=1;
    void *ptr=GetParam("chainprotocol",NULL);
    if(ptr)
    {
        if(strcmp((char*)ptr,"q3chain"))
        {
            m_IsProtocolq3chain=0;
        }
    }
...
int mc_q3chainParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string q3chainServerAddress(bool check_external_ip)
{
