...
// Copyright (c) 2009-2016 The q3chain developers
...
#include "filters/q3chainfilter.h"
...
typedef struct Cq3chainTxDetails
{    
...
bool Cq3chainTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool Cq3chainTxDetails::IsRelevantEntity(uint160 hash)
{
...
void Cq3chainTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
void Cq3chainTxDetails::SetAffectedLibrary(void *entity)
{
    uint160 hash=0;
    memcpy(&hash,entity,MC_AST_SHORT_TXID_SIZE);
...
bool mc_CompareAssetQuantities(Cq3chainTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void q3chainTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool q3chainTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool q3chainTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               Cq3chainTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool q3chainTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       Cq3chainTxDetails *details,           // Tx details object
...
bool q3chainTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          Cq3chainTxDetails *details,   
                                          string& reason)      
{
...
bool q3chainTransaction_CheckCachedScript(const CTransaction& tx, 
...
void q3chainTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        Cq3chainTxDetails *details)      
{
...
bool q3chainTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool q3chainTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               Cq3chainTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!q3chainTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!q3chainTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!q3chainTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            q3chainTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool q3chainTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   Cq3chainTxDetails *details,   
                                                   string& reason)      
{
...
bool q3chainTransaction_CheckVariableUpdateDetails(const CTransaction& tx,
                                                   mc_EntityDetails *entity,
                                                   int offset, 
                                                   int vout,
                                                   Cq3chainTxDetails *details,   
...
bool q3chainTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                Cq3chainTxDetails *details,   
...
bool q3chainTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           Cq3chainTxDetails *details,   
                                           string& reason)      
{
...
bool q3chainTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           Cq3chainTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!q3chainTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
       ...
       (entity.GetEntityType() == MC_ENT_TYPE_LIBRARY))                         // Asset, variable or library update
    {
        if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
        {
            if(!q3chainTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
            {
                return false;            
            }
        }
        ...
        {
            if(!q3chainTransaction_CheckVariableUpdateDetails(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
    ...
    else
    {
        ...
        {
            if(!q3chainTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!q3chainTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool q3chainTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             Cq3chainTxDetails *details,   
                                             string& reason)      
{
...
bool q3chainTransaction_CheckIfLibraryApproval(unsigned char* ptr,
                                                  Cq3chainTxDetails *details)
{
    mc_EntityDetails entity;
...
bool q3chainTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
        ...
        {   
            if(mc_gState->m_TmpScript->GetPermission(&type,&from,&to,&timestamp) == 0) // Grant script
            {
                ...
                if(type)
                {
                    ...
                    Cq3chainAddress address;
                    unsigned char* ptr=NULL;
                    flags=MC_PFL_NONE;
                    if(lpKeyID != NULL)
                    {
                        address=Cq3chainAddress(*lpKeyID);
                    ...
                    {
                        flags=MC_PFL_IS_SCRIPTHASH;
                        address=Cq3chainAddress(*lpScriptID);
                            ...
                            if(details->vInputDestinations[i] != 0)
                            {
                                if( ( (type & (MC_PTP_ADMIN | MC_PTP_MINE)) == 0) || details->vInputCanGrantAdminMine[i] || (entity.GetEntityType() > 0) )
                                {
                                            ...
                                            from,to,timestamp,flags,1,offset) == 0)
                                    {
                                        if(type & MC_PTP_FILTER)
                                        {
                                            q3chainTransaction_CheckIfLibraryApproval(ptr,details);
                                        }
                                        fAdminFound=true;
                                    }
                                }
...
bool Is_q3chainLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    q3chainTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool q3chainTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               Cq3chainTxDetails *details,   
                                               string& reason)      
{
...
bool q3chainTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               Cq3chainTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!q3chainTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool q3chainTransaction_ProcessTokenIssuance(const CTransaction& tx,
                                                int offset,  
                                                bool accept,
                                                int vout,
...
bool q3chainTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        q3chainTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!q3chainTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!q3chainTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!q3chainTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!q3chainTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!q3chainTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }                                                                                        
    ...
    if(mc_gState->m_Features->NFTokens())
    {
        for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
        {
            ...
            if(!q3chainTransaction_ProcessTokenIssuance(tx,offset,accept,vout,0,details,reason))
            {
                return false;                
            }        
        }    
...
bool q3chainTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        Cq3chainTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!q3chainTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t q3chainTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool q3chainTransaction_CheckMandatoryFee(Cq3chainTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = q3chainTransaction_OffchainFee(details->total_offchain_size);
...
bool q3chainTransaction_CheckLicenseTokenDetails(Cq3chainTxDetails *details,     // Tx details object
...
bool q3chainTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
            ...
            (details->new_entity_type != MC_ENT_TYPE_LIBRARY) )
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    q3chainTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!q3chainTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
    ...
    if(err == 0)
    {
        if(mc_gState->m_Features->NFTokens())
        {
            if(followon_mode & MC_ENT_FOMD_NON_FUNGIBLE_TOKENS)
            {
                ...
                {
                    q3chainTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);
                    if(!q3chainTransaction_ProcessTokenIssuance(tx,offset,accept,vout,txid,details,reason))
                    {
                        return false;                
                    }        
                }
...
bool q3chainTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 Cq3chainTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                pq3chainFilterEngine->AddFilter((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool q3chainTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         Cq3chainTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool q3chainTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        Cq3chainTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocolq3chain() == 0))
    {
        LogPrintf("Non-standard coinbase: Sigature output doesn't match protocol\n");
        return true;                                                            // Signature output should appear only if protocol=q3chain
    }
...
bool q3chainTransaction_ProcessLibraryUpdates(int offset,
                                                 Cq3chainTxDetails *details, // Tx details object                                                 
                                                 string& reason)                // Error message
{
    ...
    if(pq3chainFilterEngine)
    {    
        pq3chainFilterEngine->CheckLibraries(&(details->vAffectedLibraries),(offset >= 0));
    }
    return true;
}


bool Acceptq3chainTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    if(mc_gState->m_NetworkParams->IsProtocolq3chain() == 0)                 
    {
        return true;
    }    
    
    details.fCheckCachedScript=q3chainTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!q3chainTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!q3chainTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!q3chainTransaction_CheckOutputs(tx,inputs,offset,accept,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!q3chainTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!q3chainTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!q3chainTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!q3chainTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!q3chainTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!q3chainTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!q3chainTransaction_ProcessLibraryUpdates(offset,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
        ...
        ((flags & MC_AMT_NO_FILTERS) == 0) )
    {
        if(mc_gState->m_Features->Filters())
        {
            if(pq3chainFilterEngine)
            {
                mc_q3chainFilter* lpFilter;
                int applied=0;

                if(pq3chainFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
...
exitlbl:
    ...
    if(!accept || fReject)                                                      // Rolling back permission database if we were just checking or error occurred    
    {
        ...
        q3chainTransaction_ProcessLibraryUpdates(offset,&details,reason);
    }
