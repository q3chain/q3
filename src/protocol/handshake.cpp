...
bool q3chainNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !q3chainNode_CanConnect(pnode);
    }
...
bool q3chainNode_AcceptData(CNode *pnode)
{
    ...
    return !q3chainNode_DisconnectRemote(pnode);
}
...
bool Verifyq3chainVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!Verifyq3chainVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//        addrman.GetMCAddrMan()->Set(pfrom->addrFromVersion,pfrom->kAddrRemote);
        if(fIsVerackack)
        {
            ...
            LogPrintf("mchn: Connection from %s received on peer=%d in verackack (%s), %s, connection id: %d\n",
                    Cq3chainAddress(pubKeyHash).ToString().c_str(), pfrom->id,pfrom->addr.ToString(),pfrom->fInbound ? "Inbound" : "Outbound",(int)nonce_to_print);
                    
            if(!q3chainNode_CanConnect(pfrom))
            {
...
//            if(pfrom->fDefaultMessageStart)
    ...
    {
        Cq3chainAddress address(mapArgs["-handshakelocal"]);
    ...
    
    {
        ...
        if(pNodeStatus)
        {
            ...
            {
                pNodeStatus->sAddress=Cq3chainAddress(pkey.GetID()).ToString();
            }
        }
    }
