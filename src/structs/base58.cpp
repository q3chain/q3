// Copyright (c) 2014 The q3chain developers
...
namespace
{
class Cq3chainAddressVisitor : public boost::static_visitor<bool>
{
private:
    Cq3chainAddress* addr;

public:
    Cq3chainAddressVisitor(Cq3chainAddress* addrIn) : addr(addrIn) {}
...
/* MCHN START */
bool Cq3chainAddress::Set(const CKeyID &id,const std::vector<unsigned char>& vchV)
{
    SetData(vchV, &id, 20);
    return true;    
}
...
bool Cq3chainAddress::Set(const CKeyID& id)
{
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool Cq3chainAddress::Set(const CScriptID& id)
{
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool Cq3chainAddress::Set(const CTxDestination& dest)
{
    return boost::apply_visitor(Cq3chainAddressVisitor(this), dest);
}

bool Cq3chainAddress::IsValid() const
{
    return IsValid(Params());
}

bool Cq3chainAddress::IsValid(const CChainParams& params) const
{
...
CTxDestination Cq3chainAddress::Get() const
{
    if (!IsValid())
...
bool Cq3chainAddress::GetKeyID(CKeyID& keyID) const
{
...
bool Cq3chainAddress::GetScriptID(CScriptID& scriptID) const
{
...
bool Cq3chainAddress::IsScript() const
{
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void Cq3chainSecret::SetKey(const CKey& vchSecret)
{
...
bool Cq3chainSecret::IsValid() const
{
...
bool Cq3chainSecret::SetString(const char* pszSecret)
{
...
bool Cq3chainSecret::SetString(const std::string& strSecret)
{
