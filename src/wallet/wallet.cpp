...
string CWallet::SetDefaultKeyIfInvalid(std::string init_privkey)
{
    ...
    Cq3chainSecret vchSecret;
    if(!vchSecret.SetString(init_privkey))
    {
        if(mc_gState->m_NetworkParams->GetParam("privatekeyversion",NULL) == NULL)
        {
            return "Private key version is not set, please chose seed node running at least q3chain 1.0 beta 2";            
        }
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = Cq3chainAddress(CScriptID(redeemScript)).ToString();
...
{
    vector <Cq3chainAddress> vAddressesToImport;
    ...
    *err=MC_ERR_NOERROR;
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        ...
        {
            BOOST_FOREACH(const PAIRTYPE(Cq3chainAddress, CAddressBookData)& item, lpWallet->mapAddressBook)
            {
...
//                if(lpent->m_Entity.m_EntityType & MC_TET_CHAINPOS)
    ...
    if(vAddressesToImport.size())
    {
        ...
        {
            const Cq3chainAddress& address = vAddressesToImport[i];
            CTxDestination addressRet=address.Get();        
            const CKeyID *lpKeyID=boost::get<CKeyID> (&addressRet);
            const CScriptID *lpScriptID=boost::get<CScriptID> (&addressRet);
...
/* MCHN END */                    
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-q3chain-address
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return Createq3chainTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs, lpCoinsToUse, 
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return Createq3chainTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs,lpCoinsToUse,eErrorCode);
}

int CWallet::Selectq3chainCombineCoinsMinConf(int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
bool CWallet::Selectq3chainCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
{
//    printf("Selectq3chainCoinsMinConf\n");
    setCoinsRet.clear();
    nValueRet = 0;
...
bool CWallet::Selectq3chainCoins(const CAmount& nTargetValue, vector<COutput> &vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts, 
                                int in_selected_row,int in_asset_row,int in_preferred_row,
...
//                    if(mc_GetLE(in_amounts->GetRow(in_selected_row)+coin_id*MC_AST_ASSET_REF_SIZE,MC_AST_ASSET_QUANTITY_SIZE) == 0)// BUG??? 
    ...
    {
        if((Selectq3chainCoinsMinConf(nTargetValue, 1, 6, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            Selectq3chainCoinsMinConf(nTargetValue, 1, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            (bSpendZeroConfChange && Selectq3chainCoinsMinConf(nTargetValue, 0, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet))))
        {
            return true;
        }
...
bool CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey, string& reject_reason)
{
    {
        LOCK2(cs_main, cs_wallet);
        if(GetBoolArg("-logcommittx",true))LogPrintf("CommitTransaction: %s, vin: %d, vout: %d\n",wtxNew.GetHash().ToString().c_str(),(int)wtxNew.vin.size(),(int)wtxNew.vout.size());
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;    
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(Cq3chainAddress(address).ToString(), strPurpose))
        return false;
    LogPrint("mchn","Stored address %s in address book, account: %s, purpose: %s.\n",Cq3chainAddress(address).ToString().c_str(),strName.c_str(),strPurpose.c_str());
    return CWalletDB(strWalletFile).WriteName(Cq3chainAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = Cq3chainAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(Cq3chainAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(Cq3chainAddress(address).ToString());
}
...
{
    {
        ...
        int64_t nKeys = max(GetArg("-keypool", mc_gState->m_NetworkParams->IsProtocolq3chain() ? 1 : 100), (int64_t)0);
...
bool CWallet::GetKeyFromAddressBook(CPubKey& result,uint32_t type,const set<CTxDestination>* addresses,map<uint32_t, uint256>* mapSpecialEntity)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(Cq3chainAddress, CAddressBookData)& item, mapAddressBook)
    {
        const Cq3chainAddress& address = item.first;
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(Cq3chainAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(Cq3chainAddress(dest).ToString(), key);
}
...
bool CWalletTx::AcceptToMemoryPoolReturnReason(bool fLimitFree, bool fRejectInsaneFee,string& reject_reason)
{
    ...
    {
        pq3chainFilterEngine->SetTimeout(pq3chainFilterEngine->GetSendTimeout());
    }
    ...
    if(pq3chainFilterEngine)
    {
        pq3chainFilterEngine->SetTimeout(pq3chainFilterEngine->GetAcceptTimeout());
    }
