...
uint32_t q3chainNextChunkQueryAttempt(uint32_t attempts)
{
...
bool q3chainProcessChunkResponse(const CRelayResponsePair *response_pair,map <int,int>* request_pairs,mc_ChunkCollector* collector)
{
...
int q3chainResponseScore(mc_RelayResponse *response,mc_ChunkCollectorRow *collect_row,map<int64_t,int64_t>& destination_loads,uint32_t max_total_size)
{
    unsigned char *ptr;
    unsigned char *ptrEnd;
    unsigned char *ptrStart;
...
int q3chainCollectChunks(mc_ChunkCollector* collector)
{
...
//                    printf("coll new rsp: row: %d, id: %s, %d\n",row,collect_row->m_State.m_Request.ToString().c_str(),collect_row->m_State.m_RequestPos);
    ...
    {
        q3chainProcessChunkResponse(&(item.first),&(item.second.m_Pairs),collector);
            ...
            if(collect_row->m_State.m_Request.IsZero())
            {
                ...
                if(!collect_row->m_State.m_Query.IsZero())
                {
                    ...
                    if(query == NULL)
                    {
                        ...
                        collect_row->m_State.m_QueryNextAttempt=time_now+q3chainNextChunkQueryAttempt(collect_row->m_State.m_QueryAttempts);                                                
                ...
                if(query)
                {
                    ...
                    {
                        this_score=q3chainResponseScore(&(query->m_Responses[i]),collect_row,destination_loads,max_total_destination_size);
...
int q3chainCollectChunksQueueStats(mc_ChunkCollector* collector)
{
    int row;
    int delay;
    mc_ChunkCollectorRow *collect_row;
