        ...
        if(lpent->m_EntityType & MC_TET_CHAINPOS)
        {
            switch(lpent->m_EntityType & MC_TET_TYPE_MASK)
            {
                ...
                case MC_TET_SCRIPT_ADDRESS:
                    if(Cq3chainAddressFromTxEntity(address,lpent))
                    {
...
/*                        
                        else
                        {
                            ...
                            if(pq3chainFilterEngine->RunStreamFilters(tx,i,entity.m_EntityID,filter_block, filter_offset, 
    ...
    else
    {
        if(StoredTxDef.m_Flags & MC_SFL_NODATA)
        {
            if(StoredTxDef.m_Flags & MC_SFL_IS_ADDRESS)
            {
                ...
                {
                    ret=Cq3chainAddress((CScriptID)subkey_hash).ToString();
                }
                else
                {
                    ret=Cq3chainAddress((CKeyID)subkey_hash).ToString();                    
                }
            }
        }
        else
