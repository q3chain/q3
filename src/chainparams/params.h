...
#ifndef q3chainPARAMS_H
#define	q3chainPARAMS_H
...
typedef struct mc_Oneq3chainParam
{    
...
typedef struct mc_q3chainParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_Oneq3chainParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolq3chain;
    int m_ProtocolVersion;
    ...
    int Clone(const char *name,mc_q3chainParams *source);
    ...
    const mc_Oneq3chainParam *FindParam(const char *param);
    ...
    int IsParamUpgradeValueInRange(const mc_Oneq3chainParam *param,int version,int64_t value);
