...
int mc_Oneq3chainParam::IsRelevant(int version)
{
    int ret=1;
...
int64_t mc_q3chainParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_q3chainParams::Int64ToDecimal(int64_t value)
{
    if(value < 0)
    {
...
int64_t mc_q3chainParams::DecimalToInt64(double value)
{
    return (int64_t)(value*MC_PRM_DECIMAL_GRANULARITY+mc_gState->m_NetworkParams->ParamAccuracy());    
}

int mc_q3chainParams::GetParamFromScript(char* script,int64_t *value,int *size)
{
    char *ptr;    
    ptr=script;
    ptr+=strlen(ptr)+1;
...
double mc_q3chainParams::GetDoubleParam(const char *param)
{
...
void* mc_q3chainParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_q3chainParams::IsParamUpgradeValueInRange(const mc_Oneq3chainParam *param,int version,int64_t value)
{
...
int mc_q3chainParams::CanBeUpgradedByVersion(const char *param,int version,int size)
{
    if(m_lpIndex == NULL)
    {
...
int mc_q3chainParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_q3chainParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc_q3chainParams::Init()
{
    ...
    m_Count=sizeof(q3chainParamArray)/sizeof(mc_Oneq3chainParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((q3chainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(q3chainParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(q3chainParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_Oneq3chainParam*)mc_New(sizeof(q3chainParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,q3chainParamArray,sizeof(q3chainParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_q3chainParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_Oneq3chainParam *param;
    char *ptrData;
    int num_sets;
...
double mc_q3chainParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


const mc_Oneq3chainParam *mc_q3chainParams::FindParam(const char* param)
{
    int i;
    for(i=0;i<m_Count;i++)
    {
        if(strcmp((q3chainParamArray+i)->m_Name,param) == 0)
        {
            return q3chainParamArray+i;
        }        
    }    
    return NULL;
}

int mc_q3chainParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_q3chainParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc_q3chainParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc_q3chainParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((q3chainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((q3chainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (q3chainParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (q3chainParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((q3chainParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((q3chainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((q3chainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_q3chainParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_q3chainParams::Clone(const char* name, mc_q3chainParams* source)
{
    int err;
...
int mc_q3chainParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_q3chainParams::Validate()
{
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"q3chain") == 0)
            {
...
int mc_q3chainParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== q3chain configuration file ====\n\n");
    fprintf(fileHan,"# Created by q3chain-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"q3chaind %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun q3chain-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"q3chaind %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"q3chaind %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                ...
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence q3chain network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"q3chain-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running q3chaind for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"q3chain-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by q3chain-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by q3chaind.\n");
...
int mc_q3chainParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_q3chainParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_q3chainParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_q3chainParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_q3chainParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_q3chainParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_q3chainParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_q3chainParams::ProtocolVersion()
{
...
int mc_q3chainParams::IsProtocolq3chain()
{
    return m_IsProtocolq3chain;
}
