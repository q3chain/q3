...
def get_options():
    mc_path_default = str(Path.home() / "q3chain")
    ...
    parser.add_argument("-m", "--q3chain", metavar="DIR", default=mc_path_default,
                        help="q3chain path prefix (default: %(default)s)")
    ...
    if not Path(options.q3chain).exists():
        parser.error("{!r}: q3chain path does not exist".format(options.q3chain))

    logger.info("{} - {}".format(logger.name, parser.description))
    logger.info("  Path:     {}".format(options.q3chain))
...
def main():
    ...
    options = get_options()
    os.chdir(str(Path(options.q3chain) / "v8build" / "v8" / "out.gn" / "x64.release"))
